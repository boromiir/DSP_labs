...
//used for task 1 - oversampling with DMA
/*void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc )
{
	...
	//value = sum / BUFFER_SIZE;
	//adcBuffer[counter] = value;
	//counter++;
...
const float SINE_TABLE_HALF_VALUE = 32768;
...
void DDSAlgorithm()
{
	...
	static float phaseAccumulator = 0;
	//AM:
	phaseAccumulator += phaseStep;
	//FM:
	//phaseAccumulator += phaseStep * amplitude;

	if (phaseAccumulator >= SINE_BUF_SIZE)
		phaseAccumulator -= SINE_BUF_SIZE;

	//AM:
	float dacValue = (lookup[(int)phaseAccumulator] - SINE_TABLE_HALF_VALUE) * amplitude;
	//FM:
	//float dacValue = (lookup[(int)phaseAccumulator] - SINE_TABLE_HALF_VALUE);
	dacValue += SINE_TABLE_HALF_VALUE;
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc )
{
	float modulation = HAL_ADC_GetValue(&hadc1);
	amplitude = modulation / 4096;
}
