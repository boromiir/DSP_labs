...
void DDSAlgorithm()
{
	...
	HAL_DAC_SetValue(&hdac, DAC_CHANNEL_2, DAC_ALIGN_12B_L, lookup[(int)phaseAccumulator]);
}
...
int main(void)
{
  ...
  //HAL_DAC_Start_DMA(&hdac, DAC1_CHANNEL_2, (uint32_t*) lookup, BUFFER_SIZE, DAC_ALIGN_12B_L);

  //HAL_ADC_Start_DMA(&hadc1, (uint32_t *)dmaBuffer, BUFFER_SIZE);
